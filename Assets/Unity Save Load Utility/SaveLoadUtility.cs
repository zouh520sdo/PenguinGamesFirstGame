...
public class SaveLoadUtility : MonoBehaviour {
    ...
    // Super
    [HideInInspector]
    private List<Type> necessarySuperClass = new List<Type>()
    {
        typeof(Wandable)
    };
	...
	public SceneObject PackGameObject(GameObject go, ObjectIdentifier oi) {
		...
		foreach(object component in components) {
			...
			if(compType == typeof(Transform) || compType.IsSubclassOf(typeof(MonoBehaviour))/*compType.BaseType == typeof(MonoBehaviour)*/) {
	...
	private void GetValues(object baseInstance, ref Dictionary<string,object> baseDict, bool checkForPropertySelector) {
        ...
        /*
        foreach (Type tt in necessarySuperClass)
        {
            Debug.Log("[Checking Required super class]   " + tt.Name);
            Debug.Log("[Checking base super class]   " + baseInstanceType.Name);
            Debug.Log(typeof(TestCube).IsSubclassOf(typeof(MonoBehaviour)));
            if (baseInstanceType.IsSubclassOf(tt))
            {
                superTypes.Add(tt);
                Debug.Log("[Required super class]   " + tt.Name);
            }
        }*/
		...
		List<FieldInfo> fields = new List<FieldInfo>(baseInstanceType.GetFields(flags));
        // Check for required super class
        foreach (Type sp in superTypes)
        {
            FieldInfo[] tempFields = sp.GetFields(flags);
            if (tempFields != null)
            {
		...
		if(debugController.fieldsLength) {
			Debug.Log("[GetValues] " + "fields.Length: " + fields.Count);
		}
		...
		if(checkForPropertySelector == true) {
			List<PropertyInfo> properties = new List<PropertyInfo>(baseInstanceType.GetProperties(flags));
            // Check for required super class
            foreach (Type sp in superTypes)
            {
                PropertyInfo[] tempProperties = sp.GetProperties(flags);
            ...
            if (debugController.fieldsLength) {
				Debug.Log("[GetValues] " + "properties.Length: " + properties.Count);
			}
